...
export class Message extends Model {
    ...
    static getMessagesCollectionReference(chatId){
    ...
    static async sendAudio(chatId, from, file, metadata, photo){
        ...
        try {
            ...
            let msgRef = await Message.send(chatId, from, 'audio', '');

            let fileUrl = await Message.upload(file, from);
    ...
    static async sendContact(chatId, from, contact){

        try {

            await Message.send(chatId, from, 'contact', contact);
    ...
    static async sendDocument(chatId, from, file, filePreview, info){

        try {

            let msgRef = await Message.send(chatId, from, 'document', '');

            let previewUrl = await Message.upload(filePreview, from);
    ...
    static sendImage(chatId, from, files){

        let promises = [];
        
        [...files].forEach( async (file) => {

            let url = await Message.upload(file, from);

            //Message.send(chatId, from, 'image', url); /*uploadTask.snapshot.downloadURL*/

            promises.push(Message.send(chatId, from, 'image', url));
    ...
    static send(chatId, from, type, content){
        ...
        return new Promise( async (resolve, reject) => {
            ...
            try {
                ...
                let result = await Message.getMessagesCollectionReference(chatId).add({
                    content,
                    timeStamp: new Date(),
                    status: 'wait',
                    from,
                ...
                await msgRef.set({ status: 'sent' }, { merge: true });
